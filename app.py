...
# --- DATOS INICIALES (Basados exactamente en tu Excel) ---
...
def borrar_en_github(tipo, valor, nombre_proyecto_app):
    ...
    try:
        ...
        if nombre_proyecto_app in datos:
            ...
            for proj_github in list(datos.keys()):
        ...
        if not nombre_github:
            return False, f"❌ No se encontró el proyecto en GitHub. Proyectos disponibles: {list(datos.keys())}"
...
with tab1:
    if not conceptos_list:
        ...
        # ELIMINAR SINCRONIZADO CON GITHUB
